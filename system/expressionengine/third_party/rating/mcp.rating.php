 ...
 * @version		3.3.1
 * @filesource	rating/mcp.rating.php
 */

require_once 'addon_builder/module_builder.php';

class Rating_mcp extends Module_builder_rating
{
	...
	public function rated_entries()
	{
		...
		// Make $params['channel_id'] an array if it's a string
		if( ! is_array($params['channel_id']) )
		{
			$params['channel_id'] = array($params['channel_id']);
			$this->cached_vars['selected']['channel_id'] = $params['channel_id'];
 		}
		...
		$sql .= $this->entries_keywords_search(ee()->input->get_post('keywords'), $params['keyword_search'], $params['channel_id'], 'any');
	...
	public function view_rating_comments($message='')
	{
		if (isset($_GET['msg']))
		{
			$message = lang($_GET['msg']);
		...
		$this->add_crumb( lang('rating_comments') );

		$this->cached_vars['module_menu_highlight']	= 'module_ratings';

		$rating_id = ee()->input->get_post('rating_id', TRUE);

		if( ($rating_id && ! ctype_digit($rating_id)) || $rating_id === FALSE )
		{
			$this->show_error('invalid_rating_id');
		}

		$this->cached_vars['rating_id'] = $rating_id;
		$this->cached_vars['rating_comments'] = array();

		$query = ee()->db->query("SELECT * FROM exp_rating_reviews WHERE rating_review != '' AND rating_id = " . ee()->db->escape_str($rating_id));

		if($query->num_rows > 0)
		{
			foreach($query->result_array() as $row)
			{
				$this->cached_vars['rating_comments'][$row['review_id']]['rating_comment_id'] = $row['review_id'];
				$this->cached_vars['rating_comments'][$row['review_id']]['name']              = $row['name'];
				$this->cached_vars['rating_comments'][$row['review_id']]['comment']           = $row['rating_review'];
				$this->cached_vars['rating_comments'][$row['review_id']]['comment_date']      = $row['review_date'];
			}
		}
		...
		$this->cached_vars['current_page'] = $this->view('view_rating_comments.html', NULL, TRUE);
		return $this->ee_cp_view('index.html');

	} // END view_rating_comments
	...
	public function get_ratings($entry_id='')
	{
		...
		if ( ! empty($entry_id))
		{
			$sql = "/* Rating get_ratings */ SELECT SQL_NO_CACHE SQL_CALC_FOUND_ROWS r.*, COUNT(rr.review_id) AS comment_total FROM exp_ratings r LEFT JOIN exp_rating_reviews rr ON rr.rating_id = r.rating_id AND rr.rating_review != '' WHERE r.entry_id = ".$entry_id; // $entry_id validated as digit above
		}
		else
		{
			$sql = "/* Rating get_ratings */ SELECT SQL_NO_CACHE SQL_CALC_FOUND_ROWS r.*, COUNT(rr.review_id) AS comment_total FROM exp_ratings r LEFT JOIN exp_rating_reviews rr ON rr.rating_id = r.rating_id AND rr.rating_review != '' WHERE r.entry_id != 0";
		}
		...
		if($params['rating_status'] == 'quarantined')
		{
			$sql .= " AND r.quarantine = 'y'";
		}
		elseif($params['rating_status'] == 'reported')
		{
			...
			if (sizeof($naughty) > 0)
			{
				$sql .= " AND r.rating_id IN (".implode(',', $naughty).")";
			}
		}
		elseif ( ! empty($params['rating_status']) AND $params['rating_status'] != 'all')
		{
			$sql .= " AND r.status = '".ee()->db->escape_str($params['rating_status'])."'";
		}
		...
		if ( 'all' != $params['collection'])
		{
			...
			{
				$sql .= " AND r.collection = '".ee()->db->escape_str($params['collection'])."'";
			}
		}
		...
		if ( ! empty($params['date_range']))
		{
			...
			{
				$this->cached_vars['selected']['start_date'] = $since;
				$sql .= " AND r.rating_date > $since";
			}

			if ( isset($before))
			{
				$this->cached_vars['selected']['end_date'] = $before;
				$sql .= " AND r.rating_date < $before";
			}
		}

		// --------------------------------------------
		//	Group rows (to avoid multiples because of comment_total)
		//	and Order by Entry Date - Hardcoded.
		// --------------------------------------------

		$sql .= " GROUP BY r.rating_id ORDER BY r.rating_date DESC";

		//	----------------------------------------
		//	Limit
		//	----------------------------------------

		$offset = ( ! ee()->input->get_post('row') ) ? 0 : ee()->input->get_post('row');

		$sql .= " LIMIT " . ee()->db->escape_str($offset) . ', ' . $this->row_limit;

		// --------------------------------------------
		//	Dax, Give me a sense of the Results and their Total
		// --------------------------------------------

		$data = ee()->db->query($sql)->result_array();

		$total_query	= ee()->db->query("/* Rating get_ratings total_rows */ \n SELECT FOUND_ROWS() as total_rows");

		$this->cached_vars['paginate'] = '';

		if ( $total_query->row()->total_rows > $this->row_limit )
		{
			$pagination_data = $this->universal_pagination(
				array(
	...
	public function ratings_keywords_search($keywords, $search_type = 'any')
	{
		...
		$custom_fields = "CAST(CONCAT_WS(' ', r.".implode(", r.", $custom_fields).') AS CHAR)';
		...
		{
			$sql .= " AND r.ip_address = '".ee()->db->escape_str(trim(substr(strtolower($keywords_exact_phrase), 3)))."'";
		}
		elseif(substr(strtolower($keywords_exact_phrase), 0, 4) == 'mid:')
		{
			$sql .= " AND r.rating_author_id = '".ee()->db->escape_str(trim(substr(strtolower($keywords_exact_phrase), 4)))."'";
		}
		else
		{
			switch($search_type)
	...
	public function ratings_actions()
	{
		...
		$id = ee()->input->get_post('id') == 'rating_comment_id' ? 'rating_comment_id' : 'rating_id';

		switch(ee()->input->post('action'))
		{
			case 'edit' : return $this->edit_ratings_form(); break;

			case 'delete' : return $this->delete_ratings_confirm_form($id); break;
	...
	public function delete_ratings_confirm_form($id = 'rating_id')
	{
		// --------------------------------------------
		...
		if (ee()->input->get_post($id) !== FALSE && ctype_digit(ee()->input->get_post($id)))
		{
			$_POST['selected'][] = ee()->input->get_post($id);
		}
		...
		elseif($id == 'rating_comment_id')
		{
			$query = ee()->db->query("SELECT review_id as rating_comment_id FROM exp_rating_reviews
								  WHERE rating_review != '' AND review_id IN (".implode(',', array_map('ceil', ee()->input->get_post('selected'))).")");
		}
		else
		{
			return $this->view_ratings();
		...
		foreach($query->result_array() as $row)
		{
			$this->cached_vars['delete'][] = $row[$id];
		}
		...
		$rating_lang                = $id == 'rating_id' ? lang('rating') : lang('rating_comment');
		$ratings_lang               = $id == 'rating_id' ? lang('ratings') : lang('rating_comments');
		$rating_delete_confirm_lang = $id == 'rating_id' ? lang('ratings_delete_confirm') : lang('rating_comment_delete_confirm');

		$this->cached_vars['delete_question'] = str_replace(array('%i%', '%rating%'),
															array(sizeof($this->cached_vars['delete']),
																 (sizeof($this->cached_vars['delete']) == 1) ? $rating_lang : $ratings_lang),
															$rating_delete_confirm_lang);

		$this->cached_vars['form_uri'] = $this->base.'&method=delete_ratings&id='.$id;
	...
	function delete_ratings()
	{
		...
		elseif ($id == 'rating_comment_id')
		{
			$query	= ee()->db->query("SELECT review_id FROM exp_rating_reviews
									   WHERE rating_review != '' AND review_id IN (".implode(',', array_map('ceil', ee()->input->post('delete'))).")");
			...
			ee()->db->query("DELETE FROM exp_rating_reviews WHERE review_id IN (".implode(',', $ids).")");

			$message = ($query->num_rows() == 1) ?
			str_replace( '%i%', 1, lang('rating_comment_deleted') ) :
			str_replace( '%i%', $query->num_rows(), lang('rating_comments_deleted') );
		}
		else
		{
			return $this->view_ratings();
	...
	function save_template()
	{
		...
		if (empty( $_POST['message']) OR
			empty( $_POST['subject']) OR
			! isset($_POST['template_id']) OR
			empty($_POST['template_name']) OR
			empty($_POST['template_label']))
		{
	 ...
	 * Module Upgrading
	 *
	 * Originally, the $current variable was going to be passed
	 * via parameter, but as there might
	 * be a further use for such a variable throughout the
	 * module at a later date we made it
	 * a class variable.
	 *
	 *
	 * @access	public
